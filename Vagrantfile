...
require 'fileutils'
require 'yaml'
if File.exists? 'Vagrantsettings.local.yaml'
  project_settings = YAML.load_file('Vagrantsettings.local.yaml')
elsif File.exists? 'Vagrantsettings.yaml'
...
Vagrant.configure(2) do |config|
  ...
  if project_settings['puppet_force_reinstall'] == 1
    ...
    if project_settings['puppet_development'] == 0
      config.vm.provision 'shell', path: 'https://raw.githubusercontent.com/devgateway/happy-deployer/v' + project_settings['puppet_module_version'] + '/shell/remove-modules.sh'
    elsif project_settings['puppet_development'] == 1
      config.vm.provision 'shell', path: project_settings['happy_deployer_path'] + '/shell/remove-modules.sh'
    end
  end

  # Prepare the box using a shell script.
  if project_settings['puppet_development'] == 0
    config.vm.provision 'shell', path: 'https://raw.githubusercontent.com/devgateway/happy-deployer/v' + project_settings['puppet_module_version'] + '/shell/setup-hiera.sh'
  elsif project_settings['puppet_development'] == 1
    config.vm.provision 'shell', path: project_settings['happy_deployer_path'] + '/shell/setup-hiera.sh'
  end

  # Make sure "patch" is installed.
  config.vm.provision 'shell', inline: "/usr/bin/yum install --quiet --assumeyes patch"
  ...
  if project_settings['puppet_development'] == 0
    ...
    config.vm.provision 'shell', inline: "(#{test_commands}) || (#{action_commands})"
    ...
    config.vm.provision 'shell', inline: "(#{install_command}) && (#{patch_command} || true)"
  end
  ...
  if project_settings['puppet_development'] == 0 && project_settings['puppet_manifest_file_contents']
    ...
    config.vm.provision 'shell', inline: 'mkdir -p /etc/puppet/manifests && echo "' + project_settings['puppet_manifest_file_contents'] + '" > /etc/puppet/manifests/site.pp'
  end
  ...
  config.vm.provision 'puppet' do |puppet|
    if project_settings['puppet_development'] == 0
      ...
      puppet.manifests_path = %w(vm /etc/puppet/manifests)
